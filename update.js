...
async function updateSignaturePadJs(repoList) {
  const from = "signature_pad@5.0.1";
  const to = "signature_pad@5.0.2";
  const basedir = Deno.cwd();
  for (const repoName of getRepos(repoList)) {
    $.cwd = `${basedir}/../${repoName}`;
...
async function updateTfjs(repoList) {
  const from = "@tensorflow/tfjs@4.19.0";
  const to = "@tensorflow/tfjs@4.20.0";
  const basedir = Deno.cwd();
  for (const repoName of getRepos(repoList)) {
    $.cwd = `${basedir}/../${repoName}`;
    await $`fdfind -tf -p -e js src -x sd -s ${from} ${to}`.quiet();
...
switch (Deno.args[0]) {
  ...
  case "tfjs": {
    ...
    const comment = "bump tfjs from 4.19.0 to 4.20.0";
  ...
  case "signature_pad": {
    ...
    const comment = "bump signature_pad from 5.0.1 to 5.0.2";
