...
async function updateBootstrap(repoList) {
  const from = "bootstrap@5.3.7";
  const to = "bootstrap@5.3.8";
  const basedir = Deno.cwd();
  for (const repoName of getRepos(repoList)) {
    $.cwd = `${basedir}/../${repoName}`;
...
async function updateSignaturePad(repoList) {
  const from = "signature_pad@5.0.10";
  const to = "signature_pad@5.1.0";
  const basedir = Deno.cwd();
  for (const repoName of getRepos(repoList)) {
    $.cwd = `${basedir}/../${repoName}`;
    await $`fdfind -tf -p -e js src -x sd -s ${from} ${to}`.quiet();
...
switch (Deno.args[0]) {
  ...
  case "bootstrap": {
    ...
    await build("all.lst");
    const comment = "bump bootstrap from 5.3.7 to 5.3.8";
  ...
  case "signature_pad": {
    ...
    const comment = "bump signature_pad from 5.0.10 to 5.1.0";
