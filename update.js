...
async function updateTfjs(repoList) {
  const from = "https://cdn.jsdelivr.net/npm/@tensorflow/tfjs@4.4.0";
  const to = "https://cdn.jsdelivr.net/npm/@tensorflow/tfjs@4.5.0";
  const basedir = Deno.cwd();
  for (const repoName of getRepos(repoList)) {
    Deno.chdir(`${basedir}/../${repoName}`);
...
switch (Deno.args[0]) {
  ...
  case "tfjs": {
    ...
    const comment = "bump tfjs from 4.4.0 to 4.5.0";
